...
const HotelSlideItem = ({ hotelOffer }) => {
  return (
    <div className="group flex flex-col gap-y-4">
      <Link
        href={hotelOffer?.link}
        className="rounded-t-md flex aspect-[262/280] overflow-hidden"
      >
        <Image
          src={generateImage(hotelOffer?.thumbnail)}
          alt={hotelOffer?.title}
          width={262}
          height={280}
        ...
        <div className="flex justify-between gap-x-4 text-xs lg:text-sm">
          <div className="flex gap-x-1.5">
            ...
            <p className="text-t-700">
              <span className="font-medium">({hotelOffer?.rating})</span>{" "}
              reviews
            </p>
          </div>
          <div className="text-right leading-relaxed">
            <p className="text-p-900 font-medium">
              BDT {numeral(hotelOffer?.price).format("0,0.00")}
            </p>
          </div>
        </div>
      </div>
